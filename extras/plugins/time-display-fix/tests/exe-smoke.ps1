...
$sizes=@{}
try {
 foreach($mode in @('baseline','plugin','plugin-off')){
  $script:dir="$run/$mode";New-Item -ItemType Directory "$dir/mods","$dir/local" | Out-Null
  if($mode -ne 'baseline'){Copy-Item "$root/time-display.wimod" "$dir/mods/time-display.wimod"}
  if($mode -eq 'plugin-off'){
    New-Item -ItemType Directory "$dir/mods/time-display" | Out-Null
    $saved=Get-ChildItem "$root/verification/loader-*/off-settings.xml" | Sort-Object LastWriteTime | Select-Object -Last 1
    if(!$saved){throw 'Run loader-test.ps1 first to obtain real host-persisted off settings'}
    Copy-Item $saved.FullName "$dir/mods/time-display/settings.xml"
  }
  ...
  try {
   ...
   if($mode -eq 'plugin'){
    ...
    Check ((Get-Content $log -Raw).Contains('已注册设置项：time-display-enabled')) 'actual EXE registers plugin setting'
   ...
   if($mode -eq 'plugin-off'){
    ...
    Check ($text.Contains('显示时间设置：关闭') -and !$text.Contains('注册资源 kind=5')) 'fresh actual EXE restores host-persisted off without timer'
   }
  } finally {
