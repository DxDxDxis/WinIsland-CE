...
namespace wi {
namespace {
constexpr COLORREF Background=RGB(17,18,21), Card=RGB(29,31,36), Text=RGB(236,238,244), Muted=RGB(171,178,191);
enum { Back=600, ListPage, Add, Folder, Scan, ReloadAll, Market, Filter, List, Details, Status,
       ...
       ResourceList, Value, Apply, Run };
struct Manager {
    ModLoader& loader; HWND hwnd=nullptr; HFONT font=nullptr,heading=nullptr;
    ...
    explicit Manager(ModLoader& l):loader(l){}
    ...
    void layout(){
        ...
        if(page==0){
            ...
            put(ResourceList,24,120,w-48,h-324);put(Value,24,h-188,w-48,34);
    ...
    void resourceSelection(bool update=true){uint64_t h=resourceHandle();auto it=std::find_if(state.resources.begin(),state.resources.end(),[&](auto& r){return r.handle==h;});bool valid=it!=state.resources.end();
        ...
        if(update)text(Value,valid?wide(it->value):L"");
    }
    ...
    void command(int id,int code){
        ...
        switch(id){
        case Back: if(page==0)DestroyWindow(hwnd);else pageTo(page==2?1:0);break;
        ...
        case Apply:{wchar_t value[4097]{};GetWindowTextW(item(Value),value,4097);loader.invoke(resourceHandle(),utf8(value));refresh();break;}
    ...
    void draw(DRAWITEMSTRUCT& d){
        ...
        if(d.CtlID==List){FillRect(dc,&r,card);if(d.itemID>=ids.size())return;auto it=std::find_if(state.mods.begin(),state.mods.end(),[&](auto& m){return m.id==ids[d.itemID];});if(it==state.mods.end())return;
            COLORREF color=(d.itemState&ODS_SELECTED)?RGB(46,58,78):Card;auto b=CreateSolidBrush(color);auto old=SelectObject(dc,b);auto pen=SelectObject(dc,GetStockObject(NULL_PEN));RoundRect(dc,r.left+3,r.top+2,r.right-3,r.bottom-2,int(12*dpi),int(12*dpi));SelectObject(dc,pen);SelectObject(dc,old);DeleteObject(b);
            ...
            line.top+=int(27*dpi);line.bottom=r.bottom-int(8*dpi);SetTextColor(dc,it->error.empty()?Muted:RGB(255,150,150));auto sub=wide(it->version+" · "+it->status);DrawTextW(dc,sub.c_str(),-1,&line,DT_SINGLELINE|DT_END_ELLIPSIS|DT_NOPREFIX);
        }else{FillRect(dc,&r,bg);bool disabled=d.itemState&ODS_DISABLED;COLORREF color=disabled?RGB(30,32,37):(d.itemState&ODS_SELECTED)?RGB(54,65,84):GetPropW(d.hwndItem,L"hover")?RGB(49,54,65):RGB(37,41,49);
            auto brush=CreateSolidBrush(color);auto border=CreatePen(PS_SOLID,1,RGB(68,74,86));auto old=SelectObject(dc,brush),oldPen=SelectObject(dc,border);RoundRect(dc,r.left,r.top,r.right,r.bottom,int(12*dpi),int(12*dpi));SelectObject(dc,old);SelectObject(dc,oldPen);DeleteObject(brush);DeleteObject(border);
            wchar_t label[256]{};GetWindowTextW(d.hwndItem,label,256);SetTextColor(dc,disabled?RGB(112,117,126):Text);DrawTextW(dc,label,-1,&r,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
        }
...
LRESULT CALLBACK managerProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
    ...
    case WM_DPICHANGED:{p->dpi=HIWORD(wp)/96.f;p->fonts();auto r=(RECT*)lp;SetWindowPos(hwnd,nullptr,r->left,r->top,r->right-r->left,r->bottom-r->top,SWP_NOZORDER|SWP_NOACTIVATE);p->layout();return 0;}
    case WM_GETMINMAXINFO:{auto m=(MINMAXINFO*)lp;m->ptMinTrackSize={LONG(680*p->dpi),LONG(460*p->dpi)};return 0;}
    case WM_TIMER:p->refresh();return 0;
    case WM_COMMAND:p->command(LOWORD(wp),HIWORD(wp));return 0;
    case WM_DRAWITEM:p->draw(*(DRAWITEMSTRUCT*)lp);return TRUE;
    case WM_CTLCOLORSTATIC:case WM_CTLCOLOREDIT:case WM_CTLCOLORLISTBOX:{HDC dc=(HDC)wp;SetTextColor(dc,Text);bool card=msg!=WM_CTLCOLORSTATIC||(HWND)lp==p->item(Details);SetBkColor(dc,card?Card:Background);return (LRESULT)(card?p->card:p->bg);}
    ...
    case WM_NCDESTROY:KillTimer(hwnd,1);managerWindow=nullptr;SetWindowLongPtrW(hwnd,GWLP_USERDATA,0);delete p;return 0;
    }
...
void openModManager(HWND parent,ModLoader& loader){
    ...
    auto model=new Manager(loader);managerWindow=CreateWindowExW(WS_EX_CONTROLPARENT,c.lpszClassName,L"WinIsland · 插件管理 · 1.2.6alpha-c1",WS_OVERLAPPEDWINDOW|WS_CLIPCHILDREN,left,top,width,height,parent,nullptr,c.hInstance,model);
    if(managerWindow){BOOL dark=TRUE;DwmSetWindowAttribute(managerWindow,20,&dark,sizeof(dark));ShowWindow(managerWindow,SW_SHOW);SetForegroundWindow(managerWindow);}
}
