...
bool Renderer::draw(Scene &s) {
    ...
    brush->SetColor(D2D1::ColorF(s.modTint,1.f)); target->FillGeometry(geometry.Get(),brush.Get());
    ...
    if (s.notice && s.noticeAlpha > .001 && h > mh) {
        ...
        text(s.notice->title, nx + 24, y, tw, titleH, 14, D2D1::ColorF(1, 1, 1, alpha), true, true);
        if (!s.notice->body.empty())
            text(s.notice->body, nx + 24, y + titleH + 6.4f, tw, (float)s.noticeBodyHeight, 12,
    ...
    if(s.modH>.1){
        ...
        for(auto& r:s.modResources){if(r.kind!=WI_BUTTON&&r.kind!=WI_LAYER)continue;if(row>=3)break;
            ...
            if(r.kind==WI_BUTTON){
                rounded(b,7,s.hover==(int)r.handle?D2D1::ColorF(.26f,.29f,.34f,r.alpha):D2D1::ColorF(.18f,.20f,.24f,r.alpha));
                if(s.keyboard&&s.focus==(int)r.handle){brush->SetColor(D2D1::ColorF(.7f,.8f,1.f,r.alpha));target->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(b.x+1,b.y+1,b.x+b.w-1,b.y+b.h-1),6,6),brush.Get(),1);}
                if(r.alpha>=.99f)s.buttons.emplace_back((int)r.handle,Box{b.x*islandScale,b.y*islandScale,b.w*islandScale,b.h*islandScale});
            }
            text(wide(r.label+(r.kind==WI_LAYER&&!r.value.empty()?": "+r.value:"")),b.x+6,b.y+3,b.w-12,18,11,D2D1::ColorF(1,1,1,r.alpha));
        }
        target->PopAxisAlignedClip();
    }
    target->PopLayer();
