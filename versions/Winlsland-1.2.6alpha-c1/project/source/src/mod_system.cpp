...
struct ModLoader::Impl {
    struct Record {
        Impl* host=nullptr; ModInfo info; fs::path folder,entry;
        ...
        std::string desired="enabled";
        std::map<std::wstring,std::wstring> settings;
    };
    struct Resource { ModResourceView view; WinIslandCallback callback=nullptr; void* context=nullptr; uint32_t interval=0; double next=0; };
    struct Work {std::string action,id,value; bool cascade=false; uint64_t handle=0;fs::path folder;};
    ...
    void scan() {
        ...
        for(auto& c:candidates){
            std::string id=c.package?c.package->manifest.id:"broken-"+sha256(utf8(c.path.filename().wstring())).substr(0,16);
            ...
            r.api={sizeof(WinIslandHostApi),WINISLAND_MOD_ABI,&r,&hostLog,&add,&remove,&getSetting};
            log(&r,"读取清单与模组信息");
            if(c.package){auto& m=c.package->manifest;r.info.name=m.name;r.info.description=m.description;r.info.author=m.author;r.info.version=m.version;r.info.dependencies=m.dependencies;r.info.dependencyVersions=m.dependencyVersions;r.info.packageHash=c.package->hash;r.info.signature=c.package->signature;}
            r.info.error=duplicate?"模组 ID 重复：请保留一个包后刷新":c.error;
            if(!r.info.error.empty()){r.info.status="加载失败";log(&r,r.info.error);continue;}
            r.info.valid=true;r.desired=clean(readFile(r.folder/L"host-state.txt"));if(r.desired!="disabled"&&r.desired!="unloaded")r.desired="enabled";
    ...
    bool enable(Record& r,std::set<std::string>& visiting) {
        ...
        for(auto& dep:r.info.dependencies){auto it=records.find(dep);if(it==records.end()||!enable(*it->second,visiting)){r.info.error="依赖无法启用："+dep;state(r,"加载失败");visiting.erase(r.info.id);return false;}}
        visiting.erase(r.info.id);r.info.error.clear();state(r,"正在加载");
        try {
            if(!r.dll){
                if(!r.package)throw std::runtime_error("缺少有效 .wimod 包");
                log(&r,"ZIP 解压到内部缓存；hash="+r.package->hash);r.entry=r.package->extract(cacheRoot);
                ...
                if(!abi||!factory||guardedAbi(abi)!=WINISLAND_MOD_ABI)throw std::runtime_error("DLL ABI 不兼容；需使用本版 POD 接口重新编译");
            ...
            r.info.enabled=true;r.desired="enabled";persist(r);state(r,"已启用");fade(r,true);return true;
    ...
    void disable(Record& r,bool unload) {
        ...
        if(unload){state(r,"正在卸载");destroy(r);r.desired="unloaded";state(r,"已卸载");}
        else {destroy(r);r.desired="disabled";state(r,"已禁用");}
        persist(r);
    }
    ...
    void process(Work& w) {
        if(w.action=="event"){
            ...
            for(auto& res:resources)if((res.view.kind==WI_EVENT||res.view.kind==WI_HOOK)&&res.view.key==w.id){auto it=records.find(res.view.owner);if(it!=records.end()&&it->second->accepting&&guarded(res.callback,res.context,w.value.c_str())!=0)failed.push_back(res.view.owner);}
            for(auto& id:failed){auto it=records.find(id);if(it!=records.end()){it->second->info.error="事件回调失败，已恢复该模组";disable(*it->second,false);}}
            return;
        }
        if(w.action=="scan"||w.action=="startup"){
            scan();if(w.action=="startup")for(auto& [id,r]:records)if(r->desired=="enabled"){std::set<std::string> seen;enable(*r,seen);}return;
        }
        if(w.action=="install") {
            ...
            if(it==resources.end())return;auto res=*it;auto& r=*records.at(res.view.owner);if(!r.accepting||!r.info.enabled)return;
            if(res.view.kind==WI_SETTING){if(w.value.size()>4096)throw std::runtime_error("设置过长");it->view.value=w.value;r.settings[wide(res.view.key)]=wide(w.value);saveValues(r);}
            auto beforeHandle=nextHandle,beforeSize=resources.size();
            int rc=guarded(res.callback,res.context,w.value.c_str());
            if(rc){r.info.error="插件回调异常；已停用并恢复";auto order=closeOrder(r.info.id);for(auto& id:order)disable(*records.at(id),false);state(r,"加载失败");}
        ...
        if(w.action=="enable") {auto it=records.find(w.id);if(it==records.end())throw std::runtime_error("插件不存在");std::set<std::string> seen;enable(*it->second,seen);return;}
        if(w.action=="disable"||w.action=="unload"||w.action=="reload") {
            ...
            for(auto& id:order){auto& r=*records.at(id);
                if(w.id.empty()&&w.action=="disable"&&!r.info.loaded&&!r.info.enabled)continue;
                if(r.info.enabled||id==w.id)restart.push_back(id);disable(r,w.action!="disable");}
            if(w.action=="reload") {scan();for(auto i=restart.rbegin();i!=restart.rend();++i){std::set<std::string> seen;enable(*records.at(*i),seen);}}
    ...
    void run() {
        ...
        for(;;){Work work;bool have=false;
            {std::unique_lock l(mutex);cv.wait_for(l,std::chrono::milliseconds(50),[&]{return stopping||!queue.empty();});if(stopping)break;
             if(!queue.empty()){work=queue.front();queue.pop_front();have=true;}}
            if(have){try{process(work);}catch(const winrt::hresult_error&){log(nullptr,"JSON/WinRT 操作失败");publish("操作失败：JSON/WinRT 错误");}catch(const std::exception& ex){log(nullptr,ex.what());publish(std::string("操作失败：")+ex.what());}catch(...){publish("插件异常");}
                {std::lock_guard l(mutex);published.busy=false;++published.revision;}}
        ...
        auto order=closeOrder("");for(auto& id:order){auto& r=*records.at(id);auto desired=r.desired;try{disable(r,true);r.desired=desired;persist(r);}catch(...){r.accepting=false;revoke(r);destroy(r);log(&r,"退出清理时文件写入失败");}} // shutdown preserves choices
        winrt::uninit_apartment();
    }
    bool submit(Work w){std::lock_guard l(mutex);if(stopping||published.busy)return false;published.busy=true;++published.revision;queue.push_back(std::move(w));cv.notify_all();return true;}
};
...
std::vector<std::string> ModLoader::affected(const std::string& id)const{
    ...
    std::function<void(std::string)> visit=[&](std::string target){if(!seen.insert(target).second)return;for(auto& m:s.mods)if((m.loaded||m.enabled)&&std::find(m.dependencies.begin(),m.dependencies.end(),target)!=m.dependencies.end())visit(m.id);result.push_back(target);};visit(id);return result;
}
} // namespace wi
