...
class App {
  public:
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.6alpha-c1");
    ...
    void updateInput() {
        ...
        if(!suspended){
            auto shell=inputShell(client.right);
            for(auto [id,b]:scene.buttons)if(id>=50000){
    ...
    void tick() {
        if(mods&&now()>=nextMods){nextMods=now()+.016;ModSnapshot current;if(mods->snapshotSince(modState.revision,current)){
            modState=std::move(current);scene.modResources=modState.resources;
    ...
    static LRESULT CALLBACK proc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_MOUSEACTIVATE:
            return a.hasMusic || !a.scene.modResources.empty() ? MA_ACTIVATE : MA_NOACTIVATE;
        case WM_MOUSEMOVE: {
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.6alpha-c1", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
