...
## 1.2.7alpha-r1 媒体接口适配

官方插件如需皮肤资源，先通过 `queryInterface("winisland.media", 1, ...)` 做能力发现，再以包内相对路径调用 `load/info/bind`。r1 提供真实 GIF 和 H.264 播放，但仍须读取 capabilities 与异步 info；禁用/卸载由宿主自动撤销 owner 资源。实现细节以 `media_api.h` 和《媒体资源与皮肤扩展》为准。
