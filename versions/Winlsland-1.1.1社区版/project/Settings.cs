...
namespace WinIsland
{
    internal sealed class IslandSettings
    {
        ...
        {
            if (seconds < 0.1 || seconds > 3600 || double.IsNaN(seconds) || double.IsInfinity(seconds)) return;
            if (Resident == resident && HideNative == hideNative && DwellSeconds == seconds) return;
            Resident = resident; HideNative = hideNative; DwellSeconds = seconds;
        ...
        private void Save()
        {
            ...
            try
            {
                ...
                new XDocument(new XElement("WinIsland", new XAttribute("version", "1.1"),
                    new XElement("Resident", Resident), new XElement("HideNative", HideNative),
                    new XElement("DwellSeconds", DwellSeconds.ToString(CultureInfo.InvariantCulture)))).Save(temp);
