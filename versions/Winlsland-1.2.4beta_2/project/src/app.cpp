...
class App {
  public:
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.4beta_2");
    ...
    void position() {
        ...
        SetWindowPos(inputWindow, HWND_TOPMOST, origin.x, origin.y + (int)(geo.offset * dpi), width,
                     (int)std::ceil(hostHeight * dpi), SWP_NOACTIVATE);
        if (renderer)
    ...
    void updateInput() {
        ...
        if (hasMusic && scene.music && scene.musicAlpha > .05 && !suspended) {
            ...
            double radius = std::min({scene.radius, scene.width / 2, scene.height / 2}) * dpi;
            auto shell =
                CreateRoundRectRgn((int)std::ceil(left), 0, (int)std::floor(left + scene.width * dpi) + 1,
                                   (int)std::floor(scene.height * dpi) + 1, (int)std::lround(radius * 2),
                                   (int)std::lround(radius * 2));
            ...
            inputButtons.emplace_back(5,
                                      Box{(float)(left / dpi), 0, (float)scene.width, (float)scene.height});
        }
    ...
    void applySettings() {
        ...
        if (media)
            media->configure(settings.songSource, settings.lyricSource, settings.lyricApi);
        if (telemetry)
    ...
    void retarget(double seconds) {
        ...
        target.ih = information && !hasMusic ? 29.92 : 0;
        ...
        if (scene.notice && !closingNotice) {
            ...
            target.h = target.mh + target.ih +
                       std::min(hostHeight - target.mh - target.ih, std::max(52.8, titleH + bodyH + 33.6));
            scene.noticeBodyHeight = std::max(0., target.h - target.mh - target.ih - titleH - 40);
    ...
    void tick() {
        ...
        if (dirty && !suspended) {
            // Layout/animation stays in its original units. Apply the requested
            // 95% once at the scene boundary, also used by OS input/accessibility.
            scene.width = geo.w * IslandScale;
            scene.height = geo.h * IslandScale;
            scene.radius = std::min({geo.r, geo.w / 2, geo.h / 2}) * IslandScale;
            scene.musicH = geo.mh * IslandScale;
            scene.musicAlpha = geo.ma;
            scene.noticeAlpha = geo.na;
            scene.infoH = geo.ih * IslandScale;
            ...
            if (submitted) {
                ...
                if (scene.notice && geo.na >= .95 && !closingNotice && !switching &&
                    geo.h - geo.mh - geo.ih >= 40 && visibleNotice != scene.notice->correlation) {
...
static LRESULT CALLBACK styledProc(HWND h, UINT msg, WPARAM wp, LPARAM lp, UINT_PTR, DWORD_PTR) {
    ...
    if (wcscmp(className, L"Static") == 0) {
        ...
        if (msg == WM_PAINT) {
            ...
            bool outside = id == 106 || (id >= 200 && id <= 202) || (id >= 240 && id <= 242) || id == 222;
            SetDCBrushColor(dc, outside ? RGB(246, 247, 250) : RGB(255, 255, 255));
            FillRect(dc, &r, (HBRUSH)GetStockObject(DC_BRUSH));
            bool heading = id == 200 || id == 222 || (id >= 240 && id <= 243) || id == 230 || id == 231 ||
                           id == 233 || id == 235 || (id >= 210 && id <= 216 && id % 2 == 0);
            SetBkMode(dc, TRANSPARENT);
            SetTextColor(dc, id == 106 || id == 238 || id == 244 ? RGB(177, 49, 60)
    ...
    if (msg == WM_KEYDOWN &&
        ...
        bool edit = wcscmp(className, L"Edit") == 0;
        if (!combo && !edit)
...
void App::openSettings() {
    ...
    for (int i = 0; i < 4; ++i) {
        const wchar_t *tabs[] = {L"界面通知", L"音乐歌词", L"实时信息", L"运行诊断"};
    ...
    for (int id : {100, 101, 102, 103, 120, 121, 125, 104, 105, 122, 123, 124, 108, 109, 111}) {
...
void App::layoutSettings() {
        ...
        EnumChildWindows(
            ...
            [](HWND child, LPARAM value) -> BOOL {
                ...
                bool smallText = id == 202 || id == 106 || id == 107 || id == 110 || id == 111 || id == 220 ||
                                 ...
                                 id == 236 || id == 237 || id == 238 || id == 239 || id >= 244 && id <= 246;
    ...
    const int pageHeight[] = {452, 638, 522, 466};
    ...
    auto card = [&](double y, double h) {
        settingsCards.push_back({26, (float)y, (float)(w - 52), (float)h});
    };
    ...
    if (settingsPage == 0) {
        ...
        place(243, 42, 238, inner, 40);
        field(125, 42, 284, inner);
        place(244, 42, 330, inner, 42);
        place(232, 42, 376, inner, 54);
        place(107, 42, 474, inner, 60);
        double left = std::min(212., (inner - 10) * .56);
        place(104, 42, 556, left, 38);
        place(105, 52 + left, 556, inner - left - 10, 38);
...
LRESULT CALLBACK App::settingsProc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    if (msg == WM_COMMAND && !a.settingsCreating) {
        ...
        if (id >= 250 && id <= 253 && code == BN_CLICKED) {
            for (int combo : {120, 121})
                sendSetting(a.settingsWindow, combo, CB_SHOWDROPDOWN, FALSE, 0);
            a.settingsPage = id - 250;
            a.settingsScroll = 0;
            a.wheelRemainder = 0;
            a.layoutSettings();
            for (int tab = 250; tab <= 253; ++tab)
        ...
        if ((id == 102 || id == 103) && code == EN_CHANGE)
            a.settingsChanged(id);
        if (((id == 120 || id == 121) && code == CBN_SELCHANGE) ||
            ((id == 122 || id == 123) && code == BN_CLICKED) ||
            ((id == 124 || id == 125) && code == EN_KILLFOCUS))
            a.settingsChanged(id);
    ...
    if (msg == WM_DRAWITEM) {
        ...
        if (d->CtlType == ODT_BUTTON) {
            HDC dc = d->hDC;
            RECT r = d->rcItem;
            bool nav = d->CtlID >= 250 && d->CtlID <= 253;
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.4beta_2", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
