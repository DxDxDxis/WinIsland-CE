...
bool Renderer::draw(Scene &s) {
    ...
    rounded({x, 0, w, h}, radius, D2D1::ColorF(0, 0, 0, 1));
    if (s.press > .001 && !s.music && !s.notice) {
        rounded({x + 1, 1, w - 2, h - 2}, std::max(1.f, radius - 1),
    ...
    factory->CreateRoundedRectangleGeometry(
        D2D1::RoundedRect(D2D1::RectF(x, 0, x + w, h), radius, radius), &geometry);
    ...
    if (m && s.musicAlpha > .001 && mh > 0) {
        ...
        constexpr float contentScale = MusicScale * IslandScale;
        const float x = musicOrigin / MusicScale, w = (float)s.width / contentScale,
