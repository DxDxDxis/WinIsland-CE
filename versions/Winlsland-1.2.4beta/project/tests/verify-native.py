...
def check(ok, name):
    if not ok:
        pt=W.POINT();u.GetCursorPos(C.byref(pt));s=state();rect=W.RECT()
        if s.get('MusicWindow'):u.GetWindowRect(int(s['MusicWindow']),C.byref(rect))
        raise AssertionError(name+f'; cursor={pt.x},{pt.y}; target={u.WindowFromPoint(pt)}; input={s.get("MusicWindow")}; rect={rect.left},{rect.top},{rect.right},{rect.bottom}; idle={s.get("PanelIdleSeconds")}')
    passed.append(name)
...
def wait(predicate, name, seconds=8):
    ...
    while time.monotonic()<until:
        ...
        time.sleep(.025)
    raise AssertionError(f'Timeout: {name}; pointer/state='+str({k:s.get(k) for k in ('PointerDowns','PointerUps','PointerHit','ActionCount','LastAction','MusicExpanded','HasMusic','Busy','PanelIdleSeconds')}))
...
def click_button(n):
    if n and state().get('MusicExpanded')!='True':click_button(0);settle()
...
try:
    ...
    check(abs(float(s['Width'])-414.414)<.1 and abs(float(s['Height'])-43.5456)<.1,'Compact music 414.414 x 43.5456 DIP')
    click_button(0);s=settle()
    check(s['MusicExpanded']=='True' and abs(float(s['Height'])-109.62)<.1,'Pointer expands music to 109.62 DIP')
    send('seconds=0.8');toast(2)
    s=wait(lambda s:s.get('NoticeTitle')=='数据库消息' and s.get('Animating')=='False','toast expanded')
    check(s['MusicExpanded']=='True' and float(s['NoticeTop'])==float(s['MusicHeight']) and float(s['Height'])>109.62,'Notice extends downward from expanded music')
    ...
    check(s['MusicExpanded']=='True' and abs(float(s['Height'])-109.62)<.1,'Notice dismissal restores expanded music')
    check(before is None or s.get('NoticesShown')==before,'Unchanged notification payload is not repeated')
    x,y=map(float,s['Button0'].split(','));r=W.RECT();u.GetWindowRect(int(s['MusicWindow']),C.byref(r));u.SetCursorPos(r.left+round(x),r.top+round(y))
    ...
    # Genuine hover keeps the panel open, including a stationary pointer.
    x,y=map(float,s['Button0'].split(','));r=W.RECT();u.GetWindowRect(int(s['MusicWindow']),C.byref(r))
    u.SetCursorPos(r.left+round(x),r.top+round(y));time.sleep(5.3)
    check(state()['MusicExpanded']=='True','Stationary hover suspends automatic contraction')
    u.mouse_event(2,0,0,0,0);time.sleep(.1);u.SetCursorPos(50,850);time.sleep(5.3)
    check(state()['MusicExpanded']=='True','Held pointer outside the panel cannot force contraction')
    u.mouse_event(4,0,0,0,0);time.sleep(.1)
    time.sleep(3);check(state()['MusicExpanded']=='True','Release starts a fresh five-second idle period')
    s=wait(lambda s:s.get('MusicExpanded')=='False' and s.get('Animating')=='True','idle contraction is animated',4)
    s=settle();check(s['MusicPlaying']=='True' and abs(float(s['Height'])-43.5456)<.1,'Idle timeout preserves playback and returns to compact music')
    click_button(0);s=settle();check(s['MusicExpanded']=='True','Pointer reopens panel after automatic contraction')
    ...
    if args.fixture:
        ...
        settle()
        if state().get('MusicExpanded')!='True':click_button(0);settle()
        click_button(2)
