...
class App {
  public:
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.4beta");
    ...
    void retarget(double seconds) {
        ...
        target.r = hasMusic ? (scene.expanded ? 23.1 : 21) * MusicScale : 14.96;
    ...
    void command(const std::string &c) {
        ...
        if (c == "music-stop") {
            ...
            settings.lyricSource = std::clamp(std::stoi(c.substr(13)), 0, 4);
            saveSettings();
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.4beta", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
