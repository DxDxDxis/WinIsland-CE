...
struct PlayerLyricReader {
    ...
    void apply(Music &m) {
        ...
        if(retainedKey!=m.key() || retainedDocument!=m.lyrics || !m.lyrics || m.lyricState=="disabled")lastVerifiedLine.clear();
        retainedKey=m.key();
        ...
        for(auto &[hwnd,v]:views) {
            ...
            if(IsIconic(top)||!IsWindowVisible(top)) {
                m.lyricStatus=m.lyrics->provider+L"；歌词已保留，播放器未提供后台进度，同步暂不可用";
                continue; // CEF freezes its current-line DOM while minimized.
            }
            ...
            if(matches) {
                    ...
                    m.lyricSyncSource="player_accessibility_line";m.lyricState="player_line";
                    m.lyricStatus=m.lyrics->provider+L"；跟随播放器当前行（未提供秒级进度）";
                    return;
            }
        }
    }
...
void Media::configure(int song, int lyric, const std::wstring &api) {
    std::lock_guard lock(mu);
    if (song == songSource && lyric == lyricSource && api == lyricApi)
        return;
