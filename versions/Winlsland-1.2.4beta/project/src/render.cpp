...
bool Renderer::draw(Scene &s) {
    ...
    if (s.press > .001 && !s.music && !s.notice) {
        rounded({x + 1, 1, w - 2, h - 2}, std::max(1.f, (float)s.radius - 1),
                D2D1::ColorF(1, 1, 1, (float)s.press * .09f));
        brush->SetColor(D2D1::ColorF(1, 1, 1, (float)s.press * .25f));
        target->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF(x + 1, 1, x + w - 1, h - 1),
                                                       (float)s.radius - 1, (float)s.radius - 1),
                                     brush.Get(), 1);
    }
    ...
    if (m && s.musicAlpha > .001 && mh > 0) {
        ...
        constexpr float barWidth = 2.1f / MusicScale, maxBarHeight = 18.f / MusicScale;
        float fpsAlpha = (float)s.fpsAlpha, pingAlpha = (float)s.pingAlpha;
        float metricWidth = 78 * (fpsAlpha + pingAlpha);
        float gap = std::clamp((w - 18.65f - 70 - metricWidth - 96 / MusicScale - MusicBarCount * barWidth) /
                                   (MusicBarCount - 1),
                               .8f / MusicScale, 2.4f / MusicScale);
        float region = MusicBarCount * barWidth + (MusicBarCount - 1) * gap;
        float barLeft = x + w - 18.65f - region;
        float metricRight = barLeft - 8;
        if (metricRight - metricWidth - x - 62 < 64 / MusicScale) {
            fpsAlpha = 0;
        ...
        s.barArea = {(barLeft - x) * MusicScale, (48.384f - maxBarHeight) * MusicScale / 2,
                     region * MusicScale, maxBarHeight * MusicScale};
        s.songTextRight = (58 + textW) * MusicScale;
        s.barGap = gap * MusicScale;
        ...
        for (size_t i = 0; i < MusicBarCount; i++) {
            ...
            rounded({barLeft + i * (barWidth + gap), (48.384f - bh) / 2, barWidth, bh}, barWidth / 2,
                    D2D1::ColorF(.765f, .839f, .973f, alpha));
        }
    ...
    if (s.infoH > .001) {
        ...
        target->PushAxisAlignedClip(D2D1::RectF(x, 0, x + w, (float)s.infoH), D2D1_ANTIALIAS_MODE_ALIASED);
        metrics(x + (w + combined) / 2, 7, 10.5f, 75, (float)s.infoAlpha, (float)s.fpsAlpha,
                (float)s.pingAlpha);
        target->PopAxisAlignedClip();
    }
    mh += (float)s.infoH;
