...
$bundlePath=Join-Path $src 'embedded_components.bundle'
...
if errorlevel 1 exit /b 1
cl /nologo /MP /std:c++20 /EHsc /MT /O2 /Gy /Gw /utf-8 /permissive- /Zc:__cplusplus /W3 /DUNICODE /D_UNICODE /Fo"$obj\\" /Fe"$OutputDirectory\WinIsland-1.2.6alpha-c2.exe" core.cpp monitor.cpp media.cpp music_sources.cpp telemetry.cpp audio.cpp notifications.cpp qq_notifications.cpp render.cpp accessibility.cpp animation_runtime.cpp embedded_components.cpp mod_system.cpp scene_store.cpp mod_package.cpp mod_dependency.cpp mod_ui.cpp mod_tests.cpp app.cpp tests.cpp "$obj\miniz.obj" "$obj\miniz_tinfl.obj" "$obj\miniz_zip.obj" "$obj\app.res" "$obj\qjs.lib" /link /SUBSYSTEM:WINDOWS /MANIFEST:NO /OPT:REF /OPT:ICF user32.lib gdi32.lib shell32.lib shlwapi.lib ole32.lib oleaut32.lib runtimeobject.lib windowsapp.lib d3d11.lib d2d1.lib dwrite.lib dxgi.lib dcomp.lib windowscodecs.lib dwmapi.lib shcore.lib wtsapi32.lib bcrypt.lib advapi32.lib xmllite.lib psapi.lib mmdevapi.lib crypt32.lib comctl32.lib uxtheme.lib oleacc.lib uiautomationcore.lib gdiplus.lib winhttp.lib iphlpapi.lib ws2_32.lib
$testBuild
exit /b %errorlevel%
"@
...
Get-Item -LiteralPath (Join-Path $OutputDirectory 'WinIsland-1.2.6alpha-c2.exe')|Select-Object FullName,Length
