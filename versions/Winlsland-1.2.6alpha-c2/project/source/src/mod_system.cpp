...
#include "scene_host.inc"
    ...
    void process(Work& w) {
        ...
        if(w.action=="install") {
            ...
            if(res.view.kind==WI_SETTING){if(!validSetting(res.view,w.value))throw std::runtime_error("插件设置无效：请检查类型、数值范围或选项");it->view.value=w.value;r.settings[wide(res.view.key)]=wide(w.value);saveValues(r);}
    ...
    void run() {
        ...
        for(;;){Work work;bool have=false;
            ...
            if(have){try{process(work);}catch(const winrt::hresult_error&){log(nullptr,"JSON/WinRT 操作失败");publish("操作失败：JSON/WinRT 错误");}catch(const std::exception& ex){log(nullptr,ex.what());publish(std::string("操作失败：")+ex.what());}catch(...){publish("插件异常");}
                {std::lock_guard l(mutex);if(work.action!="event"&&work.action!="scene-input"&&work.action!="scene-draw")published.busy=false;++published.revision;}}
