...
class App {
  public:
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.6alpha-c2");
    ...
    void publishOpenScene(){
        ...
        uint32_t flags=(!scene.music&&!scene.notice?WI_IDLE:0)|(scene.music?WI_MUSIC:0)|(scene.music&&scene.music->playing?WI_PLAYING:0)|(!scene.lyric.empty()&&scene.music?WI_LYRIC:0)|(scene.notice?WI_NOTICE:0)|(scene.expanded?WI_IS_EXPANDED:0)|(settingsWindow?WI_SETTINGS_OPEN:0)|(reducedMotion?WI_REDUCED_MOTION:0)|(renderer->fallback()?WI_SOFTWARE_RENDERER:0);
    ...
    bool openSceneInput(HWND h,UINT msg,WPARAM wp,LPARAM lp){
        ...
        if(msg==WM_MOUSEMOVE){if(hit!=sceneHover){send(sceneHover,WI_POINTER_LEAVE);send(hit,WI_POINTER_ENTER);sceneHover=hit;scene.hover=hit?(int)hit:-1;dirty=true;}send(scenePressed?scenePressed:hit,WI_POINTER_MOVE);}
        if(msg==WM_MOUSELEAVE){send(sceneHover,WI_POINTER_LEAVE);sceneHover=0;}
        if(msg==WM_LBUTTONDOWN&&hit){scenePressed=hit;SetCapture(h);SetFocus(h);if(sceneFocus!=hit){send(sceneFocus,WI_FOCUS_LOST);sceneFocus=hit;scene.focus=(int)hit;send(hit,WI_FOCUS_GAINED);}send(hit,WI_POINTER_DOWN);}
        if(msg==WM_LBUTTONUP){auto pressed=scenePressed;scenePressed=0;send(pressed,WI_POINTER_UP);if(pressed&&pressed==hit){send(hit,WI_POINTER_CLICK);}if(pressed)ReleaseCapture();}
        if(msg==WM_MOUSEWHEEL)send(hit,WI_POINTER_WHEEL);
        if(msg==WM_CAPTURECHANGED)scenePressed=0;
        if(msg==WM_KILLFOCUS){send(sceneFocus,WI_FOCUS_LOST);sceneFocus=0;}
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.6alpha-c2", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
