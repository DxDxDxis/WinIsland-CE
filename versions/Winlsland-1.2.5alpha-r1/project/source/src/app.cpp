...
struct Geo {
    ...
    double w = 180.4, h = 29.92, r = 14.96, mh = 0, ma = 0, na = 0, offset = 0, ih = 0, ia = 0;
};
...
class App {
  public:
    ...
    int settingsPage = 0;
    int settingsTransitionFrom = 0, settingsTransitionDirection = 1;
    double settingsTransitionStart = 0;
    bool settingsTransitioning = false;
    int settingsTransitionFrame = -1;
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.5alpha-r1");
    ...
    void retarget(double seconds) {
        ...
        if (target.topRadius == to.topRadius && target.zoom == to.zoom && std::abs(target.w - to.w) < .01 && std::abs(target.h - to.h) < .01 && target.mh == to.mh &&
            ...
            target.ih == to.ih && target.ia == to.ia)
            return;
        monitor.event("动画目标更新；宽=" + std::to_string(target.w) + " 高=" + std::to_string(target.h));
        from = geo;
        to = target;
        animStart = now();
        animSeconds = reducedMotion ? std::min(seconds, .16) : seconds;
        animating = true;
        dirty = true;
    }
    ...
    void command(const std::string &c) {
        ...
        if (c == "music-stop") {
            ...
            openSettings(); settingsPage = std::clamp(std::stoi(c.substr(14)), 0, 4);
    ...
    static LRESULT CALLBACK proc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_LBUTTONUP: {
            ...
            a.pressedButton = -1;
            ReleaseCapture();
            if (pressed >= 0 && pressed < 5 && pressed == a.inputAt(x, y)) {
                SetFocus(h);
                a.action(pressed);
            }
            return 0;
...
void App::openSettings() {
    ...
    for (int i = 0; i < 5; ++i) {
        const wchar_t *tabs[] = {L"界面通知", L"音乐歌词", L"实时信息", L"运行诊断", L"尺寸位置"};
    ...
    control(settingsContent, L"STATIC", L"大尺寸与位置", 0, 260);
    ...
    for (int id : {100, 101, 102, 103, 120, 121, 125, 126, 104, 105, 122, 123, 124, 108, 109, 111, 130, 131, 132, 133, 134, 135, 136, 138, 139, 137}) {
...
void App::layoutSettings() {
    ...
    const int pageHeight[] = {452, 756, 522, 466, 1114};
    ...
    if (settingsTransitioning)
        transition = std::clamp((now() - settingsTransitionStart) / .28, 0., 1.);
    double slideOffset = settingsTransitioning ? settingsTransitionDirection * (1 - transition) * (w + 32) : 0;
    batch = BeginDeferWindowPos(64);
    auto place = [&](int id, double x, double y, double width, double h) {
        shown.push_back(id);
        double stagger = std::min(.18, (double)(shown.size() - 1) * .012);
        double enter = settingsTransitioning ? std::clamp((transition - stagger) / .82, 0., 1.) : 1;
        double localSlide = settingsTransitioning ? slideOffset * (1 - .18 * enter) : 0;
        move(settingControl(settingsContent, id), x + localSlide, y - settingsScroll + (1 - enter) * 10, width, h, SWP_SHOWWINDOW);
    };
    auto card = [&](double y, double h) {
        settingsCards.push_back({(float)(26 + slideOffset), (float)y, (float)(w - 52), (float)h});
    };
    auto field = [&](int id, double x, double y, double width) {
        settingsFrames.push_back({id, {(float)(x + slideOffset), (float)y, (float)width, 36}});
    ...
    if (settingsPage == 0) {
        ...
        card(48, 244); card(308, 364); card(688, 168); card(872, 216);
        for (int i=0; i<3; ++i) {
        ...
        place(269, 42, 890, inner, 100); place(137, 42, 1004, 180, 38);
        place(268, 42, 1051, inner, 32);
    }
...
LRESULT CALLBACK App::settingsProc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    if (msg == WM_TIMER && wp == 7 && !content) {
        double elapsed = now() - a.settingsTransitionStart;
        int frame = (int)std::floor(std::max(0., elapsed) / .03);
        if (frame == a.settingsTransitionFrame && elapsed < .28)
            return 0;
        a.settingsTransitionFrame = frame;
        if (elapsed >= .28) {
            KillTimer(h, 7); a.settingsTransitioning = false;
        }
        a.layoutSettings();
        RedrawWindow(h, nullptr, nullptr, RDW_INVALIDATE | RDW_ALLCHILDREN);
        return 0;
    ...
    if (msg == WM_COMMAND && !a.settingsCreating) {
        ...
        if (id >= 250 && id <= 254 && code == BN_CLICKED) {
            ...
            int nextPage = id - 250;
            a.settingsTransitionDirection = nextPage >= a.settingsPage ? 1 : -1;
            a.settingsTransitionFrom = a.settingsPage;
            a.settingsPage = nextPage;
            a.settingsTransitionStart = now();
            // Keep the legacy settings experience immediate and stable.  The
            // old page did not animate category changes; avoiding an extra
            // timer/layout pass also prevents transient tearing on slower
            // systems.
            a.settingsTransitioning = false;
            a.settingsScroll = 0;
            a.wheelRemainder = 0;
            a.layoutSettings();
            a.settingsTransitionFrame = -1;
            KillTimer(a.settingsWindow, 7);
    ...
    if (msg == WM_PAINT) {
        PAINTSTRUCT ps{};
        HDC dc = BeginPaint(h, &ps);
        SetDCBrushColor(dc, RGB(246, 247, 250));
    ...
    if (msg == WM_DRAWITEM) {
        ...
        if (d->CtlType == ODT_BUTTON) {
            ...
            if (nav && (d->itemState & ODS_FOCUS)) {
                // A solid inset underline is the keyboard cue. Do not draw the
                // old dotted pagination/selection indicator beneath the tabs.
                HPEN focusPen = CreatePen(PS_SOLID, 2, selected ? RGB(255, 255, 255) : RGB(61, 104, 242));
                HGDIOBJ previousPen = SelectObject(dc, focusPen);
                MoveToEx(dc, r.left + 12, r.bottom - 3, nullptr);
                LineTo(dc, r.right - 12, r.bottom - 3);
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.5alpha-r1", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
