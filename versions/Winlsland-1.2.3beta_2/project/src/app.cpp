...
class App {
  public:
    HWND hwnd = nullptr, inputWindow = nullptr, settingsWindow = nullptr, tooltip = nullptr;
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.2.3beta_2");
    ...
    static LRESULT CALLBACK proc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_LBUTTONUP: {
            ...
            a.pressedButton = -1;
            ReleaseCapture();
            if (pressed >= 0 && pressed == a.inputAt(x, y)) {
                SetFocus(h);
                a.action(pressed);
            }
            return 0;
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.2.3beta_2", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
