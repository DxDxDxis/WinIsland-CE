            ...
            try {
                ...
                if (!captures.contains(s.pid)) {
                    ...
                    try {
                        ...
                        monitor.event("进程音频回环不可用；10秒后重试，不模拟音量");
                        retry[s.pid] = now() + 10;
                        continue;
                    }
                }
                ...
                if (FAILED(s.volume->GetMute(&mute)) || FAILED(s.volume->GetMasterVolume(&gain)) ||
                    FAILED(s.endpoint->GetMute(&outMute)) ||
                ...
                if (!silent && state == AudioSessionStateActive)
                    peak = std::max(peak, measured[s.pid] * out);
            } catch (...) {
