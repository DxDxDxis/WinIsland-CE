...
void Notices::run() {
    ...
    bool ready = false, healthy = false;
    long long version = -1;
            ...
            try {
                ...
                if (v != version) {
                    ...
                    while ((rc = sql->step(stmt)) == 100) {
                        ...
                        if ((ready || (uint64_t)n.arrival >= since) && parseToast(payload, n))
