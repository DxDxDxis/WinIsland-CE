export interface Settings {
  resident: boolean; hideNative: boolean; seconds: number; fps: number;
...
export interface Resource { handle: string; owner: string; label: string; value: string; kind: number; type: number; minimum: number; maximum: number; choices: string[] }
...
export type Params = { revision?: number; patch?: Partial<Settings>; id?: string; action?: string; cascade?: boolean; key?: string; handle?: string; value?: string };
export type Reply = ({ ok: true } & Partial<Payloads[Command]>) | { ok: false; code: string; error: string };
export interface SettingsApi { call(id: string, command: Command, params: Params): Promise<Reply>; cancel(id: string): void; close(): void }
