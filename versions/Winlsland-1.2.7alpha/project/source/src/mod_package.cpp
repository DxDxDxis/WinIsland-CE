...
namespace wi {
namespace {
constexpr size_t MaxPackage=128*1024*1024,MaxFile=64*1024*1024,MaxExpanded=256*1024*1024;
...
ModPackage::ModPackage(const fs::path& source):impl(std::make_unique<Impl>()),path(fs::absolute(source)){
    ...
    parseModManifest(impl->read(L"mod.json"),manifest);
...
void ModPackage::validate(){for(size_t i=0;i<impl->entries.size();++i)if(!impl->entries[i].info.m_is_directory)impl->read(i);impl->checkSignature();if(impl->byName.contains(L"signature.p7s"))signature="签名有效（Windows 信任链）";}
