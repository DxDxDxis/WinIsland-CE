...
构建：1.2.7alpha-r1-dynamic-media-20260914。生命周期 ABI 0x00010002、场景 ABI 1 保持；本次从 1.2.7alpha 独立副本扩展媒体和捕获输入。旧 c2 和 alpha 源码只读。
...
Image 节点现在可绑定宿主持有的静态PNG/JPEG/BMP、GIF动画和H.264视频。新增WI_MEDIA_FIT（stretch/contain/cover）与WI_MEDIA_CROP（归一化源矩形）；新增拖动start/update/end/cancel事件。对应函数表、预算、线程和真实验证见媒体资源与皮肤扩展、SDK接口参考及本版报告。根scale/rotation/shadow仍不支持，原文所述子节点变换不能推广为任意整窗变换。
