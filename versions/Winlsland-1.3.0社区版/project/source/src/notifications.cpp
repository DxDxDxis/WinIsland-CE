...
Notices::Notices(HWND h, const fs::path &p)
    : owner(h),
      path(p.empty() ? legacyDataDir().parent_path() / L"Microsoft/Windows/Notifications/wpndatabase.db" : p),
      started(fileTime()) {
...
void ToastHelper::enable(bool yes) {
    ...
    if (CreateProcessW(nullptr, cmd.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, dataDir().c_str(), &si,
                       &pi)) {
        process = pi.hProcess;
