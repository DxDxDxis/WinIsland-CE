...
struct ModInfo {
    ...
    std::string packageFile,packageHash,signature,entry,gameVersion;
    double apiVersion=0;
    uint32_t actualAbi=0;
    bool loaded=false,enabled=false,valid=false;
    uint64_t loads=0,unloads=0;
};
struct ModSnapshot {
    struct MediaView {WiMedia handle;std::string owner;WiMediaInfo info;};
    std::vector<MediaView> media;
