...
import { registerPanel, findControl, coreControls, showPanel, selectTab, disposeMotion, navigationSnapshot, restoreNavigationScroll, transitionPlugins } from './motion.js';
...
window.addEventListener('beforeunload', () => { saveNavigation(); disposed = true; switchQueue.clear(); switchIntent.clear(); pluginList.dispose(); disposeMotion(); clearInterval(pollTimer); for (const id of outstanding) window.winIsland.cancel(id); });
