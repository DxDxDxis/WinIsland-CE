...
export const motionDuration = (name: string) => reduced.matches ? 0 : parseFloat(tokens.getPropertyValue('--motion-' + name));
...
function clearMorph() {
  ...
  nav.inert = false; morph = undefined;
}
function completeMorph() {
  const state = morph; if (!state) return;
  cancelMorphAnimation(); host.dataset.pageState = state.opened ? 'plugins' : 'settings';
...
export function transitionPlugins(opened: boolean, settings: HTMLElement, plugins: HTMLElement, entry: HTMLElement, finish: () => void, resize = false) {
  ...
  const target = contentBounds(), anchor = entry.getBoundingClientRect();
  ...
  if (!layer) {
    ...
    layer.setAttribute('role', 'region'); layer.setAttribute('aria-label', '插件模组管理');
  ...
  morph = { opened, settings, plugins, entry, finish };
  ...
  shellAnimation = shell.animate([{ transform: transform(from), borderRadius: radius(from, previousCorner ?? (opened ? 8 : 12)) }, { transform: transform(to), borderRadius: radius(to, opened ? 12 : 8) }], options);
  const clip = (r: DOMRect) => `inset(${Math.max(0, r.top - target.top)}px ${Math.max(0, target.right - r.right)}px ${Math.max(0, target.bottom - r.bottom)}px ${Math.max(0, r.left - target.left)}px round 10px)`;
  contentAnimation = layer.animate([
    { opacity: oldOpacity, clipPath: clip(from) },
    { opacity: opened ? (previousRect ? oldOpacity : 0) : 0, offset: opened ? .35 : .4 },
    { opacity: opened ? 1 : 0, clipPath: clip(to) }
  ], options);
...
function resetGeometry() {
  ...
  if (reduced.matches) { if (shell) completeMorph(); disclosures.forEach(d => d.settle()); }
}
...
observer.observe(host); observer.observe(nav); reduced.addEventListener('change', resetGeometry);
export function disposeMotion() { clearMorph(); host.removeEventListener('wheel', blockUnderlyingScroll); host.removeEventListener('touchmove', blockUnderlyingScroll); cancelAnimationFrame(frame); observer.disconnect(); reduced.removeEventListener('change', resetGeometry); disclosures.forEach(d => d.dispose()); pages.clear(); scrolls.clear(); }
