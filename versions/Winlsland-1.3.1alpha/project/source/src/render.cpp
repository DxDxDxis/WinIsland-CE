...
bool Renderer::draw(Scene &s) {
    ...
    float backgroundAlpha=rootStyle.values.contains(WI_BACKGROUND)?(float)rootStyle.n(WI_BACKGROUND,3):1.f;
    brush->SetColor(D2D1::ColorF(s.modTint,rootAlpha*backgroundAlpha)); target->FillGeometry(geometry.Get(),brush.Get());
    if (s.press > .001 && !s.music && !s.notice && !s.transferState) {
    ...
    // Keep host scene metadata available to plugins while clipping the temporary
    // foreground. Receiving uses the root background/alpha, never a second fill.
    if (s.transferState) target->PushAxisAlignedClip(D2D1::RectF(0,0,0,0),D2D1_ANTIALIAS_MODE_ALIASED);
    ...
    if(!s.transferState)drawOpenScene(s,x*islandScale);
    else {
        target->PopAxisAlignedClip();
        target->SetTransform(islandTransform);
        s.buttons.clear();s.sceneHits.clear();
        text(s.transferState==1?L"中转文件":L"文件中转",x+14,6,w-28,22,14,D2D1::ColorF(0xf3f5fa),true);
        if(s.transferState==2){float viewport=std::max(12.f,h-58);float total=(float)textHeight(s.transferText,11,w-28);s.transferScroll=std::clamp(s.transferScroll,0.f,std::max(0.f,total-viewport));
            target->PushAxisAlignedClip(D2D1::RectF(x+14,28,x+w-14,28+viewport),D2D1_ANTIALIAS_MODE_ALIASED);text(s.transferText,x+14,28-s.transferScroll,w-28,total,11,D2D1::ColorF(0xc7d5e8));target->PopAxisAlignedClip();
            const wchar_t* labels[]={L"仅中转",L"保存并中转",L"取消"};float bw=(w-36)/3;
            for(int i=0;i<3;++i){Box b{x+10+i*(bw+8),h-25,bw,21};rounded(b,7,D2D1::ColorF(s.keyboard&&s.focus==700+i?0x5688cb:i==1?0x2763b9:0x343d50));text(labels[i],b.x+6,b.y+2,b.w-12,18,10,D2D1::ColorF(0xffffff));s.buttons.push_back({700+i,{b.x*islandScale,b.y*islandScale,b.w*islandScale,b.h*islandScale}});}
        }
    }
    target->PopLayer();
