...
class App {
  public:
    ...
    std::unique_ptr<TransferStore> transfer;
    std::unique_ptr<TransferWidget> transferWidget;
    int receiving=0; bool transferEnabled=false,transferRetargeting=false;
    ...
    ~App() {
        ...
        jobs.finish();
        transferWidget.reset();transfer.reset();
        native.enable(false);
        notices.reset();
        qqNotices.reset();
        media.reset();
    ...
    void startup(const fs::path &dir, bool software) {
        ...
        if (!diagnostic) { if(!wi::prepareEmbeddedComponents(false))throw std::runtime_error("未完成安装，程序没有启动或创建空数据目录"); }
        else setDataRoot(dir);
        if(GetEnvironmentVariableW(L"WINISLAND_PERF_DIR",nullptr,0)>0)perf.start(dataDir()/L"logs"/L"performance");
        config = (diagnostic ? dir : dataDir()) / L"settings.xml";
        store = (diagnostic ? dir : dataDir()) / L"lyrics";
        settings.load(config);
        transfer=std::make_unique<TransferStore>(dataDir());transferEnabled=transfer->enabled();
        mods = std::make_unique<ModLoader>(dataDir() / L"mods",true,diagnostic?dir:dataDir());
        if (observation && !fs::exists(config)) settings.load(legacyDataDir() / L"settings.xml");
        ...
        tray.hIcon = wc.hIcon;
        wcscpy_s(tray.szTip, L"WinIsland 1.3.1alpha 社区版");
        ...
        transferWidget=std::make_unique<TransferWidget>(*transfer,inputWindow,[this](int state,const std::wstring& text){
            if(receiving==state&&scene.transferText==text)return;
            bool changed=receiving!=state;if(changed)scene.transferScroll=0;receiving=state;scene.transferState=state;scene.transferText=text;
            if(state){pressAt=0;scene.press=0;scene.holdTriggered=false;queue.clear();holdUntil=0;closingNotice=bool(scene.notice);switching=awaitHold=false;}
            transferRetargeting=true;if(changed)onMusic(rawMusic);retarget(.48);transferRetargeting=false;dirty=true;
        },[this]{openSettings();});
    ...
    void updateInput() {
        ...
        shape.push_back(transferEnabled);
        ...
        if(!suspended&&transferEnabled){auto shell=inputShell(client.right);CombineRgn(region,region,shell,RGN_OR);DeleteObject(shell);inputButtons.insert(inputButtons.begin(),{6,Box{(float)((client.right-scene.width*dpi)/2/dpi),0,(float)scene.width,(float)scene.height}});}
        if(!suspended){
            auto shell=inputShell(client.right);
            for(auto [id,b]:scene.buttons)if(id>=50000 || (id>=700&&id<=702) || (id>=101&&id<=104)){
    ...
    void applySettings() {
        if (!settings.showMessages) {queue.clear();holdUntil=0;switching=awaitHold=false;if(scene.notice)closingNotice=true;}
    ...
    void retarget(double seconds) {
        ...
        if(receiving){double factor=receiving==1?2.:3.;target.w=std::min(180.4*wx*factor,(workWidth-16)/target.zoom);target.h=std::min(29.92*hy*IdleHeightScale*factor,(workHeight-16)/target.zoom);target.r=18;target.ma=target.na=target.ia=target.modH=target.ih=target.mh=0;target.offset=0;}
        if (settings.radiusMode != 0 && !receiving && !scene.expanded && (!scene.notice || closingNotice))
        ...
        if(receiving||transferRetargeting)animSeconds=transferMotionSeconds(animSeconds);
        animating = true;
        dirty = true;
    }
    void onMusic(std::shared_ptr<Music> m) {
        ...
        if (m && m->playing)
            ...
            pausedAt = now();
        bool show = settings.showMusic && !receiving && m && (m->playing || !pausedAt || now() - pausedAt < 10);
        ...
        if (presence) {
            ...
            SetWindowLongPtrW(inputWindow, GWL_EXSTYLE,
                              (show||receiving) ? style & ~WS_EX_NOACTIVATE : style | WS_EX_NOACTIVATE);
        }
    ...
    void enqueue(Notice n) {
        ...
        if (suspended || !settings.showMessages || receiving) {
    ...
    void tick() {
        ...
        if(transferWidget){auto enabled=transfer->enabled();if(enabled!=transferEnabled){transferEnabled=enabled;inputShape.clear();dirty=true;}
            RECT rect;GetWindowRect(hwnd,&rect);auto center=(rect.left+rect.right)/2;rect.left=center-(LONG)(scene.width*dpi/2);rect.right=rect.left+(LONG)(scene.width*dpi);rect.bottom=rect.top+(LONG)(scene.height*dpi);
            transferWidget->sync(rect,180.4*scaleX*dpi,29.92*IdleHeightScale*scaleY*dpi,reducedMotion,TopArcBaseline*settings.topArcScale*std::min(scaleX,scaleY));}
        ...
        if (dirty && !suspended) {
            ...
            if(!receiving&&sizeMode==WI_PLUGIN_MANAGED){scene.width=std::max(1.,island.n(WI_RECT,2));scene.height=std::max(1.,island.n(WI_RECT,3));}
            else if(!receiving&&sizeMode==WI_INTRINSIC){for(auto& raw:scene.openPlan.created){auto node=scene.openPlan.resolve(raw);if(node.n(WI_VISIBLE,0,1)&&node.n(WI_LAYOUT)){scene.width=std::max(scene.width,node.n(WI_RECT)+node.n(WI_RECT,2));scene.height=std::max(scene.height,node.n(WI_RECT,1)+node.n(WI_RECT,3));}}}
            ...
            if(!receiving&&island.values.contains(WI_POSITION)){islandX=(int)std::lround(island.n(WI_POSITION)-(cw-scene.width*dpi)/2);y=(int)island.n(WI_POSITION,1);}
            RECT r;
    ...
    void action(int id) {
        if(id>=700&&id<=702){transferWidget->receiverAction(id-700);return;}
        ...
        jobs.post([this, operation, overrideFolder, environment = env.str()] {
            ...
            try {
                if (operation == 0) {
                    ...
                    monitor.sample();
                    fs::path dir =
                        overrideFolder.empty() ? dataDir() / L"logs" / L"daxian日志" : overrideFolder;
                    try {
    ...
    void command(const std::string &c) {
        ...
        } else if (c.rfind("transfer-enter=",0)==0) { transferWidget->diagnosticDrop(wide(c.substr(15)),1);
        } else if (c.rfind("transfer-drop=",0)==0) { transferWidget->diagnosticDrop(wide(c.substr(14)),2);
        } else if (c=="transfer-leave") { transferWidget->diagnosticDrop({},0);
        } else if (c=="transfer-reference") { transferWidget->receiverAction(0);
        } else if (c=="transfer-save") { transferWidget->receiverAction(1);
    ...
    void diagnostics() {
        ...
        s << "PointerDowns=" << pointerDowns << "\nPointerUps=" << pointerUps
          ...
          << "\nAnimationSeconds=" << animSeconds
          << "\nReceiving=" << receiving << "\nShowMusic=" << truth(settings.showMusic) << "\nShowMessages=" << truth(settings.showMessages) << "\nTransferEnabled=" << truth(transferEnabled)
    ...
    static LRESULT CALLBACK proc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_MOUSEACTIVATE:
            return a.receiving || a.hasMusic || !a.scene.sceneHits.empty() || !a.scene.modResources.empty() ? MA_ACTIVATE : MA_NOACTIVATE;
        case WM_MOUSEMOVE: {
        ...
        case WM_LBUTTONUP: {
            ...
            a.pressedButton = -1;
            ReleaseCapture();
            if (pressed >= 0 && (pressed < 5 || (pressed>=700&&pressed<=702) || pressed>=50000) && pressed == a.inputAt(x, y)) {
                SetFocus(h);
                a.action(pressed);
            }
            return 0;
    ...
    static LRESULT CALLBACK inputProc(HWND h, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        if(app&&app->receiving&&msg==WM_MOUSEWHEEL){app->scene.transferScroll=std::max(0.f,app->scene.transferScroll-GET_WHEEL_DELTA_WPARAM(wp)/120.f*24);app->dirty=true;return 0;}
        if(app&&app->receiving&&msg==WM_KEYDOWN){if(wp==VK_ESCAPE){app->transferWidget->receiverAction(2);return 0;}if(wp==VK_TAB){app->scene.focus=700+(app->scene.focus-700+1+3)%3;app->scene.keyboard=true;app->dirty=true;return 0;}if(wp==VK_RETURN||wp==VK_SPACE){app->action(app->scene.focus>=700&&app->scene.focus<=702?app->scene.focus:700);return 0;}}
        if(app&&!app->receiving&&app->openSceneInput(h,msg,wp,lp))return 0;
        switch (msg) {
        case WM_NCHITTEST:
        case WM_MOUSEACTIVATE:
        case WM_MOUSEMOVE:
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    try {
        if (argc == 5 && std::wstring(argv[1]) == L"--native-helper")
        ...
        else if (argc == 3 && std::wstring(argv[1]) == L"--storage-test")
            result = wi::storagePathTest(argv[2]);
        else if (argc == 3 && std::wstring(argv[1]) == L"--transfer-widget-test")
            result = wi::transferWidgetTest(argv[2]);
            ...
            bool installedTest = argc == 3 && std::wstring(argv[1]) == L"--verify-installed";
            if(installedTest){wi::setInstallerTestHome(argv[2]);if(!wi::prepareEmbeddedComponents(false))throw std::runtime_error("隔离安装未完成");dir=wi::dataDir();}
        ...
        MessageBoxW(nullptr, wi::wide(e.what()).c_str(), L"WinIsland 1.3.1alpha 社区版", MB_ICONERROR);
        result = 1;
    }
    LocalFree(argv);
    if (graphicsToken)
