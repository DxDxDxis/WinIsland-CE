...
void parseModManifest(const std::string& bytes,ModManifest& out){
    try{
        ...
        out.apiVersion=j.HasKey(L"apiVersion")&&j.GetNamedValue(L"apiVersion").ValueType()==JsonValueType::Number?j.GetNamedNumber(L"apiVersion"):0;
        out.entry=field(j,L"entry");out.gameVersion=field(j,L"gameVersion");
        if(out.apiVersion!=1)throw std::runtime_error("API 版本不兼容（需要 1）");
