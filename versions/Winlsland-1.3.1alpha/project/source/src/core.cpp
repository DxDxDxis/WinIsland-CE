...
fs::path legacyDataDir() {
    wchar_t p[MAX_PATH];
    SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, p);
    return fs::path(p) / L"WinIsland";
}
namespace { fs::path runtimeDataRoot; }
bool dataRootReady(){return !runtimeDataRoot.empty();}
fs::path dataDir() { if(runtimeDataRoot.empty())throw std::runtime_error("应用数据目录尚未初始化");return runtimeDataRoot; }
void setDataRoot(const fs::path& root) {
    auto normalized=fs::absolute(root).lexically_normal();
    if(!runtimeDataRoot.empty()&&runtimeDataRoot!=normalized)throw std::runtime_error("当前进程已固定数据目录");
    fs::create_directories(normalized/L"temp");runtimeDataRoot=normalized;
    // Explicit module paths remain authoritative; inherited temp paths keep
    // third-party runtime scratch files away from the EXE/current directory.
    SetEnvironmentVariableW(L"TEMP",(normalized/L"temp").c_str());
    SetEnvironmentVariableW(L"TMP",(normalized/L"temp").c_str());
}
std::vector<wchar_t> childEnvironment(const fs::path& base) {
    // Restrict only child runtime storage, preserving network/proxy/PATH settings.
    std::map<std::wstring,std::wstring> values;auto block=GetEnvironmentStringsW();
    if(block){for(auto p=block;*p;p+=wcslen(p)+1){std::wstring item=p;auto eq=item.find(L'=',item[0]==L'='?1:0);if(eq!=item.npos){auto key=item.substr(0,eq);CharUpperBuffW(key.data(),(DWORD)key.size());values[key]=item.substr(eq+1);}}FreeEnvironmentStringsW(block);}
    for(auto [key,name]:std::vector<std::pair<std::wstring,std::wstring>>{{L"APPDATA",L"roaming"},{L"LOCALAPPDATA",L"local"},{L"TEMP",L"temp"},{L"TMP",L"temp"}}){auto path=base/name;fs::create_directories(path);values[key]=path.wstring();}
    std::vector<wchar_t> result;for(auto& [k,v]:values){auto s=k+L"="+v;result.insert(result.end(),s.begin(),s.end());result.push_back(0);}result.push_back(0);return result;
}
std::string fileSha256(const fs::path& path) {
    std::ifstream f(path,std::ios::binary);if(!f)throw std::runtime_error("无法读取文件进行校验："+utf8(path.wstring()));
    ...
    BCryptCreateHash(a,&h,nullptr,0,nullptr,0,0);std::array<char,65536> buffer;std::array<UCHAR,32> digest{};
    while(f){f.read(buffer.data(),buffer.size());if(f.gcount())BCryptHashData(h,(PUCHAR)buffer.data(),(ULONG)f.gcount(),0);}
    auto failed=f.bad();BCryptFinishHash(h,digest.data(),32,0);BCryptDestroyHash(h);BCryptCloseAlgorithmProvider(a,0);
    if(failed)throw std::runtime_error("文件读取中断");std::ostringstream out;for(auto c:digest)out<<std::uppercase<<std::hex<<std::setfill('0')<<std::setw(2)<<(int)c;return out.str();
}
fs::path exePath() {
    wchar_t p[32768];
...
void writeAtomic(const fs::path &p, const std::string &s) {
    ...
    t += L".tmp-"+std::to_wstring(GetCurrentProcessId())+L"-"+std::to_wstring(++sequence);
    {
    ...
    HANDLE flushed=CreateFileW(t.c_str(),GENERIC_WRITE,FILE_SHARE_READ,nullptr,OPEN_EXISTING,0,nullptr);
    if(flushed!=INVALID_HANDLE_VALUE){FlushFileBuffers(flushed);CloseHandle(flushed);}
    if (!MoveFileExW(t.c_str(), p.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {std::error_code ec;fs::remove(t,ec);throw std::runtime_error("file replace");}
}
...
void Settings::load(const fs::path &p) {
    try {
        ...
        if (m.contains(L"ShowMusic")) showMusic = m[L"ShowMusic"] != L"false";
        if (m.contains(L"ShowMessages")) showMessages = m[L"ShowMessages"] != L"false";
        // Only an explicit manual value migrates. Old automatic/system state is not a preference.
        reduceMotion = m.contains(L"ManualReducedMotion") && m[L"ManualReducedMotion"] == L"true";
...
void Settings::save(const fs::path &p) const {
    std::ostringstream s;
    s << "<?xml version=\"1.0\" encoding=\"utf-8\"?><WinIsland version=\"1.3.1alpha\"><Resident>"
      << (resident ? "true" : "false") << "</Resident><HideNative>" << (hideNative ? "true" : "false")
      << "</HideNative><ShowMusic>" << (showMusic ? "true" : "false") << "</ShowMusic><ShowMessages>" << (showMessages ? "true" : "false") << "</ShowMessages><ManualReducedMotion>" << (reduceMotion ? "true" : "false") << "</ManualReducedMotion><DwellSeconds>" << seconds << "</DwellSeconds><FrameRate>" << fps
