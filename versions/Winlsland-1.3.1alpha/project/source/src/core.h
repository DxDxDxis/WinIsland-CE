...
// Runtime paths are pinned once per process after installation/diagnostic setup.
fs::path dataDir();
bool dataRootReady();
fs::path legacyDataDir();
void setDataRoot(const fs::path& root);
std::vector<wchar_t> childEnvironment(const fs::path& runtimeRoot);
...
struct Settings {
    bool resident = true, hideNative = true, showMusic = true, showMessages = true, reduceMotion = false;
    double seconds = 4;
    int fps = 0;
    int songSource = 0;  // 0 SMTC first with title fallback, 1 SMTC only, 2 window title only
    int playerFilter = 0; // 0 automatic, 1 NetEase, 2 QQ Music, 3 Soda Music
    ...
    double topArcScale = 1.0; // relative to fixed 4-DIP outward shoulder baseline
    double islandZoom = 1.5, widthRatio = 1., heightRatio = 1., dpiCorrection = 1.;
    bool topAttach = true;
    int radiusMode = 0; // 0 automatic, 1 semicircle, 2 two-thirds (clamped to half the shortest edge)
    std::wstring monitorDevice; // empty: current monitor; otherwise MONITORINFOEX device name
    void resetLayout() {
        islandZoom = 1.5; widthRatio = heightRatio = dpiCorrection = 1.;
        topAttach = true; radiusMode = 0; monitorDevice.clear(); topArcScale = 1.0;
    }
...
class Media {
  ...
  public:
    ...
    void configure(int song, int lyric, const std::wstring &api, int player = 0);
};
