...
#include <wincodec.h>
namespace wi {
// Single non-accumulating scale for all music geometry, hit testing and animation targets.
inline constexpr float MusicScale = .9f;
// One scale relative to the pre-capsule layout; independent of monitor DPI.
inline constexpr float IslandScale = .95f;
inline constexpr double NoticeScale = .8, IdleHeightScale = .9;
struct Box {
...
struct SceneHit { WiElement id; Box bounds; D2D1_MATRIX_3X2_F inverse; float width,height; bool block; };
struct Scene {
    ScenePlan openPlan;
    std::vector<SceneNode> hostNodes,visibleNodes;
    std::vector<SceneHit> sceneHits;
    WiSceneSnapshot publicState{sizeof(WiSceneSnapshot),1};
    std::vector<ModResourceView> modResources;
    double modH=0;
    unsigned modTint=0;
    double contentScale = IslandScale, topRadius = 0;
    double width = 180.4 * IslandScale, height = 29.92 * IslandScale,
           radius = 14.96 * IslandScale, musicH = 0, musicAlpha = 0, noticeAlpha = 0,
           noticeTextWidth = 132, noticeBodyHeight = 0;
    bool expanded = false, keyboard = false, busy = false;
    int focus = 0, hover = -1;
    std::shared_ptr<Music> music;
    std::shared_ptr<Notice> notice;
    std::wstring lyric, feedback, transferText, accessibleTitle = L"WinIsland 音乐与通知";
    int transferState=0;float transferScroll=0;
...
class Renderer {
    ...
    struct MediaTexture {std::shared_ptr<const MediaFrame> frame;ComPtr<ID2D1Bitmap> bitmap;};
    std::map<WiElement,MediaTexture> mediaTextures;
    HDC memory = nullptr;
    HBITMAP bitmap = nullptr, old = nullptr;
    void drawOpenScene(Scene&,float left);
    void releaseSurface();
    void init(bool);
