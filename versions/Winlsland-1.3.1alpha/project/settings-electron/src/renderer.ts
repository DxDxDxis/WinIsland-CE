...
import { registerPanel, findControl, coreControls, showPanel, selectTab, disposeMotion, navigationSnapshot, restoreNavigationScroll, transitionPlugins, setReducedMotion } from './motion.js';
...
const fields: Field[] = [
  ...
  { key: 'showMessages', label: '在灵动岛显示消息', hint: '仅控制灵动岛消息；与隐藏 Windows 自带通知独立。关闭后丢弃待显示消息。', page: 0, type: 'check' },
  ...
  { key: 'showMusic', label: '在灵动岛显示音乐', hint: '关闭音乐卡片与歌词显示，不暂停或改变播放器。', page: 1, type: 'check' },
  ...
  { key: 'reduceMotion', label: '降低动画', hint: '手动控制，默认关闭；开启后直接切换导航与下拉选项，不跟随系统设置。', page: 4, type: 'check' },
...
function build() {
  ...
  const entries = make('div', '', 'page-entries'); el('panel-4').append(entries);
  const open = make('button', '插件管理', 'page-entry'); open.id = 'plugins-open'; open.onclick = () => {
    if (pluginPage) return;
    if (el('content').dataset.pageState !== 'closing') { savedFocus = document.activeElement as HTMLElement; savedScroll = el('content').scrollTop; } pluginPage = true;
    transitionPlugins(true, el('panel-' + page), el('plugins'), open, () => el('plugins-back').focus({ preventScroll: true }));
    void action(readPlugins);
  }; entries.append(open);
  const plugins = make('section'); plugins.id = 'plugins'; plugins.dataset.fullWindow = 'true'; registerPanel(plugins);
  plugins.append(make('h2', '插件模组管理'), make('p', '点击模组查看详情。禁用与卸载停止实例，保留插件包、设置和日志。'));
  let preparingReturn = false;
  const back = make('button', '返回设置'); back.id = 'plugins-back'; back.onclick = async () => {
    if (!pluginPage || preparingReturn) return; preparingReturn = true;
    try { await syncUnderlyingSettings(); } catch (e) { showError(e); } finally { preparingReturn = false; }
    if (disposed) return; pluginPage = false;
    el('content').scrollTop = savedScroll;
    transitionPlugins(false, el('panel-' + page), plugins, open, () => { el('content').scrollTop = savedScroll; (savedFocus?.isConnected ? savedFocus : open).focus({ preventScroll: true }); saveNavigation(); });
  }; plugins.append(back);
  ...
  filter.onchange = () => { selectedFilter = filter.value; pluginList.setFilter(selectedFilter); }; const filterRow = make('div', '', 'actions'); filterRow.append(filterLabel, filter); plugins.append(filterRow);
  const list = make('div'); list.id = 'mods-list'; plugins.append(list);
  pluginList = new PluginList(list, {
    run: action, operate: modAction, logs: async id => (await call('mods.log', { id })).text, controls,
    save: async (r, value) => { await waitOperation(await call('mods.invoke', { handle: r.handle, value })); return modState?.resources.find(x => x.owner === r.owner && x.key === r.key && x.kind === r.kind); }
  });
  transferPage = new TransferPage(params => call('transfer.call', params), () => { navigate(4); return el('panel-4'); }, syncUnderlyingSettings); entries.append(transferPage.entry);
...
async function modAction(actionName: string, id = '') {
  let cascade = false;
  if (['disable', 'unload', 'reload', 'reinstall'].includes(actionName)) {
    const affected = await call('mods.affected', { id });
    if (!id || affected.ids.length > 1 || actionName === 'reinstall') {
      const reinstall = actionName === 'reinstall' ? '重装来源：' + (modState?.mods.find(m => m.id === id)?.path || id) + '。重新校验此原包并释放到新缓存，不选择替换文件；保留设置及原启用状态，失败时尝试恢复旧实例。' : '';
      cascade = await confirm(reinstall + '将影响以下模组：' + (affected.ids.join('、') || '无运行中的模组') + '。操作保留所有模组文件和用户数据。'); if (!cascade) throw Error('已取消操作，模组状态未改变');
    }
  }
...
function renderPlugins() { if (modState) pluginList.update(modState); }
...
async function syncUnderlyingSettings() {
  const r = await call('settings.read');
  const changed = fields.filter(f => !settings || settings[f.key] !== r.settings[f.key]).map(f => f.key);
  settings = r.settings; revision = r.revision; displayValues(changed);
}
async function refresh() {
  try {
    ...
    // Content height is final only after capabilities/monitor values load.
...
window.addEventListener('beforeunload', () => { saveNavigation(); disposed = true; switchQueue.clear(); switchIntent.clear(); pluginList.dispose(); transferPage.dispose(); disposeMotion(); clearInterval(pollTimer); for (const id of outstanding) window.winIsland.cancel(id); });
window.addEventListener('keydown', e => { if (e.key === 'Escape' && !e.defaultPrevented && !document.querySelector('select:open') && !el<HTMLDialogElement>('confirm').open) { if (pluginPage) el('plugins-back').click(); else if (el('content').dataset.pageState === 'closing') el('plugins-open').click(); else window.winIsland.close(); } });
...
const pollTimer = setInterval(async () => {
  ...
  try { await readMedia(); if (pluginPage) { await readPlugins(); await syncUnderlyingSettings(); } if (!connected) await refresh(); connected = true; }
