...
(async () => {
  ...
  // Retire the known preview worker left by older merge-style staging copies.
  fs.rmSync(path.join(staging, 'dist', 'preview-worker.js'), { force: true });
  ...
  fs.writeFileSync(path.join(staging, 'package.json'), JSON.stringify({ name: 'winisland-settings', author: 'WinIsland Community', productName: 'WinIslandSettings', version: '1.3.1-alpha', main: 'dist/main.js' }));
  const outputs = await packager({ dir: staging, name: 'WinIslandSettings', executableName: 'WinIslandSettings', platform: 'win32', arch: 'x64',
    ...
    electronZipDir: path.resolve('../dependency-cache/electron'), tmpdir: path.resolve('../build-electron/tmp'), appVersion: '1.3.1-alpha', buildVersion: '1.3.1.0', win32metadata: {ProductName:'WinIsland 1.3.1-alpha 社区版',FileDescription:'WinIsland 1.3.1-alpha 社区版设置'}, download: { cacheRoot: path.resolve('../dependency-cache/electron') } });
