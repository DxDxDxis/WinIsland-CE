...
if(!$SkipNative){& "$PSScriptRoot/source/build.ps1" -BuildTools $BuildTools -OutputDirectory "$PSScriptRoot/build-1.3.1alpha" *> "$PSScriptRoot/verification/build-native.log"}
Copy-Item -LiteralPath "$PSScriptRoot/build-1.3.1alpha/WinIsland-1.3.1alpha.exe" -Destination "$PSScriptRoot/release/WinIsland-1.3.1alpha.exe" -Force
Get-FileHash -Algorithm SHA256 "$PSScriptRoot/release/WinIsland-1.3.1alpha.exe","$PSScriptRoot/release/settings/WinIslandSettings.exe" | ConvertTo-Json | Set-Content "$PSScriptRoot/verification/release-hashes.json"
