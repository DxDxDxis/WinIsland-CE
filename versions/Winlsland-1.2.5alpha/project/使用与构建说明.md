# WinIsland 1.2.5alpha

当前最新构建：`1.2.5alpha-top-docked-20260912-r3`。默认按本轮修改前尺寸放大 1.5 倍，并贴合显示器工作区顶部。外壳为黑色横向半胶囊：上沿平直、两侧对称、下方大圆角。尺寸、设置、命中测试和验证范围见 [大尺寸贴顶更新与验证](docs/大尺寸贴顶更新与验证.md)。下文歌词能力和限制继续适用。

基于原 `1.2.4beta_2` 的独立分支，原版本目录未覆盖。当前成品在 `release/1.2.5alpha-top-docked/`，运行其中 `WinIsland-1.2.5alpha.exe`。分发时复制整个目录，不能只复制主 EXE；`lyric-provider` 是歌词后端的运行依赖。
...
```powershell
.\build.ps1 -BuildTools E:\WinIslandBuildTools -OutputDirectory .\release\1.2.5alpha-top-docked -IncludeTests
.\build-qcloud.ps1 -BuildTools E:\WinIslandBuildTools -OutputDirectory .\release\1.2.5alpha-top-docked
```
