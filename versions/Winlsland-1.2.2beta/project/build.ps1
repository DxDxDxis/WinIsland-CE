...
$artifacts = if ($OutputDirectory) { [IO.Path]::GetFullPath($OutputDirectory) } else { Join-Path $project 'artifacts' }
...
$references = @('System.dll', 'System.Core.dll', 'System.Drawing.dll', 'System.Windows.Forms.dll', 'System.Xaml.dll', 'System.Xml.dll', 'System.Xml.Linq.dll', 'System.Runtime.WindowsRuntime.dll')
$compilerArgs = @('/nologo', '/target:winexe', '/platform:x64', '/optimize+', '/warn:4', '/utf8output', '/codepage:65001',
    "/out:$artifacts\WinIsland-1.2.2beta.exe", "/win32manifest:$project\app.manifest", "/win32icon:$iconPath", "/resource:$iconPath,WinIsland.ico")
...
foreach ($reference in @('System.Runtime', 'System.Runtime.InteropServices.WindowsRuntime', 'System.Threading.Tasks', 'System.Collections')) {
    $assembly = Get-ChildItem -LiteralPath (Join-Path $env:WINDIR "Microsoft.NET\assembly\GAC_MSIL\$reference") -Recurse -Filter "$reference.dll" | Select-Object -First 1
    if (-not $assembly) { throw "Missing Windows framework reference: $reference" }
    $compilerArgs += "/reference:$($assembly.FullName)"
}
foreach ($reference in @('Windows.Foundation.winmd', 'Windows.Media.winmd', 'Windows.Storage.winmd')) {
    $compilerArgs += "/reference:$env:WINDIR\System32\WinMetadata\$reference"
}
...
Get-Item -LiteralPath (Join-Path $artifacts 'WinIsland-1.2.2beta.exe') | Select-Object FullName, Length, LastWriteTime
