...
QVariantMap Request::createRequest(
    ...
    // WinIsland: preserve real transport diagnostics separately from the API business code.
    answer["http_status"] = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
    answer["network_error"] = int(reply->error());
    ...
    QNetworkProxy proxy)
{
    ...
    if (!reply->isFinished()) eventLoop.exec(); // avoid missing an already completed reply
